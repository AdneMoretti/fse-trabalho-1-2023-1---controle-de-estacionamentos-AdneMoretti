...
def read_status(config_file): 
    while(True): 
        if(GPIO.input(config_file['input'][1]["porta"])==1):
            GPIO.output(config_file["output"][4]["porta"], GPIO.HIGH)
        if(GPIO.input(config_file['input'][1]["porta"])==1):
            GPIO.output(config_file["output"][4]["porta"], GPIO.LOW)

#         if GPIO.input(config_file['']) == GPIO.HIGH or GPIO.input(sensor2_pin) == GPIO.HIGH:
...
#     elif GPIO.input(sensor2_pin) == GPIO.HIGH and GPIO.input(sensor1_pin) == GPIO.HIGH:
#         print("Carro descendo do 2º para o 1º andar")

# Verifica a ordem dos sensores acionados repetidamente
